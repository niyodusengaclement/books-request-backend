...
export default class GeneralController {
  ...
  static async findAllRequests(req, res) {
    try {
      ...
      if (req.query.type === "others")
        return GeneralController.otherBooksRequests(req, res);

      if (role === "field" && district !== "all") {
        const all = await pastorsModel.findDistrictsRequests(
          field.toUpperCase(),
          year,
          term
        );
      ...
      if (role === "pastor") {
        const all = await pastorsModel.findPastorTotalRequestsByBook(
          id_code,
          year,
          term
        );
      ...
      if (role === "field" && district === "all") {
        const all = await pastorsModel.findCertainFieldTotalRequests(
          field.toUpperCase(),
          year,
          term
        );
  ...
  static async otherBooksRequests(req, res) {
    try {
      ...
      if (role === "field") {
        const all = await pastorsModel.FieldOtherBooksRequests(
          field.toUpperCase()
        );
        if (all.length < 1) {
          return onError(res, 404, "Record(s) Not found");
  ...
  static async findAllChurches(req, res) {
    try {
      ...
      if (role === "field") {
        const churches = await pastorsModel.findFieldChurches(
          field.toUpperCase()
        );
        if (churches.length < 1) {
          return onError(res, 404, "Record(s) Not found");
  ...
  static async dashboard(req, res) {
    try {
      ...
      const { role, field, id_code } = req.user;
      if (role === "field") {
        const nbrs = await pastorsModel.findTotalNumbersByField(
          field.toUpperCase(),
          year
        );
        if (nbrs.length < 1) {
  ...
  static async startAndDeadline(req, res) {
    try {
      ...
      if (role !== "union" && role !== "admin")
        return onError(res, 403, "Access denied, Admin only");
      ...
      const values = [term, year, startDate, endDate];
      if (req.query.isUpdate === "true") {
        const result = await pastorsModel.createTimetable(values);
        if (result)
          return onSuccess(res, 201, "Timetable updated successfully", result);
      }
      const isExist = await pastorsModel.doesTimetableExist(term, year);
      if (isExist)
        return onError(res, 409, "Timetable of this Quarter already exists");
      const result = await pastorsModel.createTimetable(values);
  ...
  static async findYears(req, res) {
    try {
      ...
      if (result.length > 0)
        return onSuccess(res, 200, "Years retrieved successfully", result);
    } catch (err) {
      return onError(res, 500, "Internal Server Error");
    }
  }
  ...
  static async findSingleRequest(req, res) {
    try {
      ...
      if (result)
        return onSuccess(res, 200, "Record retrieved successfully", result);
    } catch (err) {
      return onError(res, 500, "Internal Server Error");
    }
  }
  ...
  static async updateSingleRequest(req, res) {
    try {
      ...
      if (result)
        return onSuccess(res, 200, "Record updated successfully", result);
    } catch (err) {
      return onError(res, 500, "Internal Server Error");
    }
  }
  ...
  static async deliverBooks(req, res) {
    try {
      ...
      const { id } = req.params;
      if (role !== "field")
        return onError(res, 403, "Access denied! Accessed by Field only");
      if (!id) return onError(res, 400, "No request ID provided");
      const result = await pastorsModel.deliverBooks(id);
      if (result)
        return onSuccess(res, 200, "Books Delivered successfully", result);
    } catch (err) {
      return onError(res, 500, "Internal Server Error");
    }
  }
    ...
    try {
      ...
      if (role !== "union" && role !== "admin")
        return onError(
          res,
          403,
          "Access denied!, Accessed by Admin or Union employee only"
        );
      if (bookcategory !== "other") {
        const values = [bookcategory, bookreader, booktype, language, price];
        const result = await pastorsModel.addBook(values);
        if (result) return onSuccess(res, 201, `Book added successfully`);
      }
      const values = [bookname, description, language, price];
