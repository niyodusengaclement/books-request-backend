...
import { validateCommand, validateOtherBooksRequest, validateChurch, validateLogin } from "../validators/validators";
...
export default class PastorController {
  ...
  static async createCommand(req, res) {
    try {
      ...
      if(req.query.type === 'others') {
        ...
        const { bookid, church_id, request } = req.body;
        const body = [
          id_code,
          church_id,
          bookid,
        ...
        if(output) return onSuccess(res, 201, 'Request successfully sent', output);
      }
      ...
      if(isExist) return onError(res, 409, 'The Request already exist. Update the Request instead');
      const body = [
        id_code,
        church,
        command,
  ...
  static async getCommandPerChurch(req, res) {
    try {
      ...
      if(!year || !term ) return onError(res, 400, 'Write valid Church, year and term ');

      const details = await pastorsModel.findChurchRequests(id_code, year, term);
      if(details.length < 1) {
  ...
  static async uploadPastors(req, res) {
    try {
      ...
      if(role === 'union' || role === 'admin') {
        ...
        if(extension !== 'xlsx' && extension !== 'xlsm') return onError(res, 400, 'File should be an excel with [xlsx or xlsm] extension');
        const fileRows = [];
        const schema = {
          'PASTOR NAMES': {
            prop: 'names',
        ...
        const { rows, errors } = await readXlsxFile(req.files.list.tempFilePath, { schema });
        if(errors.length > 0) {
          const err = 'Pastors List has error(s), check the excel file header (PASTOR NAMES)';
          return onError(res, 400, err);
        }
        if(rows.length > 0) {
          for(let i = 0; i <= (rows.length -1) ; i++) {
            const nbr = Math.floor(Math.random() * (99999999999 - 10000 + 1)) + 10000;
            const data = [
              nbr,
              rows[i].names,
            ]
          ...
          return onSuccess(res, 201, 'Pastors list successfully uploaded', fileRows);
        }
  ...
  static async uploadChurches(req, res) {
    try {
      ...
      if(role === 'union' || role === 'admin') {
        ...
        const { rows, errors } = await readXlsxFile(req.files.churches.tempFilePath, { schema });
        if(errors.length > 0) {
          const err = 'Churches List has error(s), check the excel file headers (FIELD, ZONE, DISTRICT, CHURCH, PASTOR ID)';
          return onError(res, 400, err);
        }
        if(rows.length > 0) {
          const fileRows = [];
          ...
          return onSuccess(res, 201, 'Churches list successfully uploaded', fileRows);
        }
  ...
  static async addSinglePastor(req, res) {
    try {
      const { role } = req.user;
      if(role === 'union' || role === 'admin') {
        if(!req.body.pastor_names) return onError(res, 400, 'Enter the pastor names please');
        const nbr = Math.floor(Math.random() * (99999999999 - 10000 + 1)) + 10000;
        const values = [
          nbr,
          req.body.pastor_names
        ]
