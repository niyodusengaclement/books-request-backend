...
router.post(
  "/account-setup",
  userAuth.checkToken,
  PastorController.setupAccount
);
router.post(
  "/create-command",
  PastorController.createCommand
);
router.get(
  "/church-command",
  userAuth.checkToken,
  PastorController.getCommandPerChurch
);
router.get(
  "/pastor-churches",
  userAuth.checkToken,
  PastorController.getChurchesPerPastor
);
router.get("/find-books", GeneralController.findBooks);
router.get("/languages", userAuth.checkToken, GeneralController.getLanguage);
router.get(
  "/districts-and-fields",
  userAuth.checkToken,
  GeneralController.findDistrictsAndFields
);
router.get("/requests", userAuth.checkToken, GeneralController.findAllRequests);
router.get(
  "/requests/:id",
  userAuth.checkToken,
  GeneralController.findSingleRequest
);
...
router
  ...
  .get(GeneralController.checkDeadline);
router.get("/years", userAuth.checkToken, GeneralController.findYears);
router.patch(
  "/update/:id",
  userAuth.checkToken,
  GeneralController.updateSingleRequest
);
router.patch(
  "/deliver/:id",
  userAuth.checkToken,
  GeneralController.deliverBooks
);
router.post("/add-book", userAuth.checkToken, GeneralController.addBook);
router.post(
  "/upload-image",
  userAuth.checkToken,
  GeneralController.uploadImage
);
router.post(
  "/upload-pastors",
  userAuth.checkToken,
  PastorController.uploadPastors
);
router.post(
  "/add-single-pastor",
  userAuth.checkToken,
  PastorController.addSinglePastor
);
router.get("/find-pastors", userAuth.checkToken, GeneralController.findPastors);
router.post(
  "/upload-churches",
  userAuth.checkToken,
  PastorController.uploadChurches
);
router.post(
  "/add-single-church",
  userAuth.checkToken,
  PastorController.addSingleChurch
);
